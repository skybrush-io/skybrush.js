...
export type Vector2ArrayOptions = {
...
/**
 * Storage for an array of 2D vectors, backed by a Float32Array.
 */
export class Vector2Array {
  private _data: Float32Array;

  constructor(length: number, options: Vector2ArrayOptions = {}) {
    const { factory = newFloat32Array } = options;
    this._data = factory(length * 2);
  }

  static from(data: Float32Array) {
    if (data instanceof Float32Array) {
      const numRows = Math.floor(data.length / 2);
      const result = new Vector2Array(numRows);
      result._data = data.slice(0, numRows * 2);
      return result;
    }

    throw new Error('Cannot convert object to Vector2Array');
  }
   ...
   */
  at(index: number): Vector2 | undefined {
    if (index < 0) {
      index += this.length;
    }
  ...
  /**
   * Copies the vector at the specified index into a pre-allocated Vector2 object.
   */
  getInto(dest: Vector2, index: number): void {
    const i2 = index * 2;
    dest.x = this._data[i2];
    dest.y = this._data[i2 + 1];
  }
  ...
  getIntoArray(dest: number[], index: number): void {
    const i2 = index * 2;
    dest[0] = this._data[i2];
    dest[1] = this._data[i2 + 1];
  }
  ...
  /**
   * Returns the X or Y components of all vectors as a Float32Array.
   */
  private _getColumn(col: 0 | 1): Float32Array {
    const n = this._data.length;
    const result: Float32Array = new Float32Array(this.length);
    let j = 0;
    for (let i = col; i < n; i += 2) {
  ...
  set(index: number, value: Vector2): void {
    const i2 = index * 2;
    this._data[i2] = value.x;
    this._data[i2 + 1] = value.y;
  }

  /**
   * Copies the vectors in this Vector2Array into another Vector2Array, with an
   * optional stride.
   */
  copyInto(dest: Vector2Array, options: StrideOptions): void {
    const { start = 0, step = 1 } = options;
    const n = this.length;

    let srcIndex = 0;
    let destIndex = start * 2;
    const destStep = step * 2;
    for (let i = 0; i < n; i++) {
   ...
   * @returns The modified Vector2Array (this).
   */
  ...
  /**
   * Calculates the derivative of the Vector2Array where the derivative at
   ...
   * @returns a new Vector2Array containing the derivatives.
   */
  derivative(options: DerivativeOptions = {}): Vector2Array {
    ...
    const result = new Vector2Array(this.length);
    const n = this._data.length;
    const offset = 2 * numSteps;
    const end = n - offset;
    ...
    if (numRows >= 2 * numSteps + 1) {
      for (let i = 0; i < numSteps; i++) {
        for (let j = 0; j < 2; j++) {
          result._data[2 * i + j] = result._data[2 * numSteps + j];
          result._data[2 * (numRows - i - 1) + j] =
            result._data[2 * (numRows - numSteps - 1) + j];
        }
      }
    } else {
  ...
  /**
   * Slices the Vector2Array from start to end indices and returns another Vector2Array.
   * @param start - The starting index (inclusive).
   * @param end - The ending index (exclusive).
   * @returns The sliced Vector2Array.
   */
  slice(start: number, end: number): Vector2Array {
    return Vector2Array.from(this._data.slice(start * 2, end * 2));
  }

  /**
   * Converts the Vector2Array into a regular array of Vector2 objects.
   * @returns the converted array of Vector2 objects
   */
  toArray(): Vector2[] {
  ...
  /**
   * Extracts the underlying Float32Array data, invalidating the Vector2Array.
   */
    ...
    if (numRows >= 2 * numSteps + 1) {
      for (let i = 0; i < numSteps; i++) {
        for (let j = 0; j < 3; j++) {
          ...
          result._data[3 * (numRows - i - 1) + j] =
            result._data[3 * (numRows - numSteps - 1) + j];
        }
      }
    } else {
      result.fillWithScalar(0);
